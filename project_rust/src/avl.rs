...
impl<K: Ord, V> node_ptr<K, V>{
    ...
    #[inline]
    fn get_node_level(&mut self) -> i32{
        unsafe{(*self.0).level}
    }
...
impl<K: Ord + Debug + fmt::Display, V: Debug> AVLTree<K, V> {
    ...
    fn update_node_level(&mut self, node: node_ptr<K,V>){
        ...
        if !node.get_parent().get_left().is_null(){ //if the left node exists (not null)
            left = node.get_parent().get_node_level(); // get the level of the left node
            println!("{}", left);
        }
        if !node.get_parent().get_right().is_null(){ //if the right node exists (not null)
            right = node.get_parent().get_node_level(); // get the level of the right node
            println!("{}", right);
        }
    ...
    fn level_of_child_nodes(&mut self, node: node_ptr<K, V>) -> i32 {
        ...
        println!("difference of: {} ", left - right);
    ...
    fn insert_balance(&mut self, node: node_ptr<K, V>){
        ...
        match level_differance{
            -1..=1 => {println!("test1");
                return}, // if the level differance is -1, 0, or 1, no need to do anything -> balanced.
            2 => {println!("test2");
                return self.right_left_rotation(node)}, // left node level is greater than right node level, perform appropriate roatation
            -2 => {println!("test3");
                return self.left_right_rotation(node)}, // right node level is greater than left node level, perform appropriate roatation
            _ => {unreachable!()}
        }
    }
